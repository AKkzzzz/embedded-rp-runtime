...
默认向量模块关闭。开启后只继承 RP-Hub 当前的 embedding 模型和 API 配置；卡内不读取 RP-Hub 全局向量记忆，也不把向量写回角色卡 JSON。

## 历史保留与总结

卡内完整楼层仍然保存在本地，但发送给模型的历史可以设置最大保留楼层，默认 40 楼。超过保留范围的旧内容由两路补回：

1. 向量检索：按当前输入召回相关片段；
2. 历史总结：使用 `summary` route 压缩人物关系、地点、物品、承诺、未解决线索和状态变化。

总结模块默认关闭，开启后默认继承 RP-Hub 的总结模型；从首个完整回合开始按来源消息 ID 建立逐轮总结，并以可配置并发数补录遗漏。近期原文窗口保持完整，更早楼层在 Prompt 中以对应总结替换助手正文。总结存入按卡隔离的 IndexedDB；卡内以后仍可把 route 改为独立模型和独立 API 适配器。
